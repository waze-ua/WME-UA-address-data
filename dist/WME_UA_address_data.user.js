...
// @updateURL    https://github.com/waze-ua/WME-UA-address-data/raw/main/dist/WME_UA_address_data.user.js
// @downloadURL  https://github.com/waze-ua/WME-UA-address-data/raw/main/dist/WME_UA_address_data.user.js
...
(function () {
    ...
    class UAAddressData extends WMEBase {
            ...
            if (data) {
                ...
                Object.keys(data).forEach((group) => {
                    data[group].forEach((item) => {
                        ...
                        let feature = turf.polygon(polygon, {
                        ...
                        if (feature) {
                            // Apply the offset
                            feature.geometry.coordinates[0] = this.translateCoordinatesArray(feature.geometry.coordinates[0], this.settings.get('offset', 'x'), this.settings.get('offset', 'y'));
                            try {
        ...
        validateHTTPResponse(res) {
            let result = false, displayError = true, errorMsg;
            if (res) {
                switch (res.status) {
                    case 200:
                        displayError = false;
